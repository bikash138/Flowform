...
import {
  ...
  useStartSession,
  useSubmitResponse,
} from "@/hooks/user/use-public-form";
...
function inputStyles(hasError: boolean): React.CSSProperties {
  return {
...
function EndAnimation({ type }: { type: EndPageAnimation }) {
  ...
  useEffect(() => {
    ...
    type Particle = { x: number; y: number; vx: number; vy: number; alpha: number; size: number; color: string; rotation?: number; vr?: number; phase?: number; };
    ...
    if (type === "confetti") {
      for (let i = 0; i < 130; i++) {
        particles.push({ x: rand(0, W), y: rand(-H * 0.6, 0), vx: rand(-1.5, 1.5), vy: rand(2, 5), alpha: 1, size: rand(6, 13), color: pick(COLORS), rotation: rand(0, Math.PI * 2), vr: rand(-0.12, 0.12) });
      }
      ...
      const spawnBurst = (cx: number, cy: number) => {
        ...
        for (let i = 0; i < 36; i++) {
          ...
          particles.push({ x: cx, y: cy, vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed, alpha: 1, size: rand(3, 6), color });
        }
      };
      ...
      for (let i = 0; i < 28; i++) {
        particles.push({ x: rand(0, W), y: H + rand(20, 150), vx: 0, vy: -rand(1.5, 3.5), alpha: 1, size: rand(22, 50), color: pick(COLORS), phase: rand(0, Math.PI * 2) });
      }
    }
    ...
    function animate() {
      ...
      if (type === "confetti" && elapsed < ACTIVE_MS && Math.random() < 0.25) {
        particles.push({ x: rand(0, W), y: -10, vx: rand(-1, 1), vy: rand(2, 4.5), alpha: 1, size: rand(6, 13), color: pick(COLORS), rotation: rand(0, Math.PI * 2), vr: rand(-0.12, 0.12) });
      }
      ...
      for (const p of particles) {
        ...
        if (type === "confetti") {
          ctx!.translate(p.x, p.y); ctx!.rotate(p.rotation!); ctx!.fillStyle = p.color; ctx!.fillRect(-p.size / 2, -p.size / 4, p.size, p.size / 2);
          p.x += p.vx; p.y += p.vy; p.vy += 0.06; p.rotation! += p.vr!;
          ...
          p.x += p.vx; p.y += p.vy; p.vy += 0.12; p.vx *= 0.97; p.vy *= 0.97; p.alpha -= 0.013;
        } else if (type === "balloons") {
          ctx!.beginPath(); ctx!.arc(p.x, p.y, p.size, 0, Math.PI * 2); ctx!.fillStyle = p.color + "cc"; ctx!.fill();
          ctx!.beginPath(); ctx!.moveTo(p.x, p.y + p.size);
          const sway = Math.sin(p.phase! + Date.now() / 800) * 4;
          ctx!.lineTo(p.x + sway, p.y + p.size + 22); ctx!.strokeStyle = p.color + "88"; ctx!.lineWidth = 1.5; ctx!.stroke();
          p.phase! += 0.025; p.x += Math.sin(p.phase! * 0.6) * 0.4; p.y += p.vy;
          if (p.y < -p.size * 2) p.alpha -= 0.04;
        }
        ctx!.restore();
      }
      if (elapsed < TOTAL_MS || particles.length > 0) requestAnimationFrame(animate);
    }
    animate();
...
function NavbarBand({
  ...
  settings: PublicFormSettings; pageBg: string; pageFg: string;
}) {
  ...
  if (navbar.showBranding) {
    return (
      <div className="shrink-0 flex items-center gap-3 px-5 h-12 border-b" style={{ backgroundColor: pageBg, borderColor }}>
        {navbar.logoUrl && (
          <img src={navbar.logoUrl} alt={navbar.brandName} className="h-7 w-auto object-contain" />
        )}
        <span className="text-sm font-semibold" style={{ color: pageFg }}>{navbar.brandName}</span>
      </div>
    );
  }

  return (
    <div className="shrink-0 flex items-center gap-2 px-5 h-12 border-b" style={{ backgroundColor: pageBg, borderColor }}>
      <img
        src="/logo.svg"
        alt="Flowform"
...
function ProgressIndicator({
  ...
  settings: PublicFormSettings; pageIndex: number; totalPages: number; pageFg: string;
}) {
      ...
      <div className="flex items-center gap-1.5">
        {Array.from({ length: totalPages }).map((_, i) => (
          <div key={i} className="rounded-full transition-all duration-300"
            style={{ width: i === pageIndex ? "22px" : "8px", height: "8px", backgroundColor: i <= pageIndex ? "var(--primary)" : trackBg }} />
        ))}
      </div>
    );
  }
  ...
  if (style === "percentage") {
    return (
      <div className="flex items-center gap-2.5">
        <span className="text-xs font-semibold tabular-nums" style={{ color: pageFg, opacity: 0.7 }}>{pct}%</span>
        <div className="flex-1 h-1 rounded-full overflow-hidden" style={{ backgroundColor: trackBg, minWidth: "60px" }}>
          <div className="h-full rounded-full transition-all duration-500" style={{ width: `${pct}%`, backgroundColor: "var(--primary)" }} />
        </div>
      </div>
    );
  }
...
function InteractiveField({
  ...
  question: Question; value: AnswerValue; onChange: (v: AnswerValue) => void; hasError: boolean;
}) {
  const placeholder = question.placeholder ?? undefined;
  const style = inputStyles(hasError);
  const commonCls = "w-full border outline-none transition-colors";
  const inputCls = `${commonCls} h-10 px-3 text-sm`;

  switch (question.type) {
    case "short_text":
      return (
        <input type="text" value={(value as string) ?? ""} onChange={(e) => onChange(e.target.value)}
          placeholder={placeholder ?? "Short answer"} style={style} className={inputCls} maxLength={question.config?.maxLength} />
      );

    case "long_text":
      return (
        <textarea value={(value as string) ?? ""} onChange={(e) => onChange(e.target.value)}
          placeholder={placeholder ?? "Your answer…"} rows={4} style={style}
          className={`${commonCls} px-3 py-2.5 text-sm resize-none`} maxLength={question.config?.maxLength} />
      );

    case "email":
      return (
        <input type="email" value={(value as string) ?? ""} onChange={(e) => onChange(e.target.value)}
          placeholder={placeholder ?? "name@email.com"} style={style} className={inputCls} />
      );

    case "number":
      return (
        <input type="number" value={(value as string) ?? ""} onChange={(e) => onChange(e.target.value === "" ? null : Number(e.target.value))}
          placeholder={placeholder ?? "0"} min={question.config?.min} max={question.config?.max} style={style} className={inputCls} />
      );
    ...
    case "url":
      return (
        <input type="url" value={(value as string) ?? ""} onChange={(e) => onChange(e.target.value)}
          placeholder={placeholder ?? "https://example.com"} style={style} className={inputCls} />
      );

    case "date":
      return (
        <input type="date" value={(value as string) ?? ""} onChange={(e) => onChange(e.target.value)} style={style} className={inputCls} />
      );

    case "select":
      return (
        <select value={(value as string) ?? ""} onChange={(e) => onChange(e.target.value || null)}
          style={style} className={`${inputCls} appearance-none cursor-pointer`}>
          <option value="" style={{ color: "var(--form-placeholder)" }}>{placeholder ?? "Select an option…"}</option>
      ...
      return (
        <div className="flex flex-col gap-2">
          {(question.options ?? []).map((opt) => {
            ...
            return (
              <button key={opt.id} type="button" onClick={() => onChange(isSelected ? null : opt.id)}
                className="flex items-center gap-2.5 px-3 py-2.5 border transition-colors text-left w-full"
                style={{ backgroundColor: isSelected ? "var(--form-choice-selected)" : "var(--form-choice-bg)", borderColor: isSelected ? "var(--primary)" : hasError ? "#ef4444" : "var(--form-input-border)", borderRadius: "var(--form-input-radius)", color: "var(--form-label)" }}>
                <div className="size-4 rounded-full border-2 shrink-0 flex items-center justify-center"
                  style={{ borderColor: isSelected ? "var(--primary)" : "var(--form-input-border)", backgroundColor: isSelected ? "var(--primary)" : "transparent" }}>
          ...
          {(question.options ?? []).map((opt) => {
            ...
            return (
              <button key={opt.id} type="button"
                onClick={() => { const next = isChecked ? selected.filter((v) => v !== opt.id) : [...selected, opt.id]; onChange(next); }}
                className="flex items-center gap-2.5 px-3 py-2.5 border transition-colors text-left w-full"
                style={{ backgroundColor: isChecked ? "var(--form-choice-selected)" : "var(--form-choice-bg)", borderColor: isChecked ? "var(--primary)" : hasError ? "#ef4444" : "var(--form-input-border)", borderRadius: "var(--form-input-radius)", color: "var(--form-label)" }}>
                <div className="size-4 rounded shrink-0 border-2 flex items-center justify-center"
                  style={{ borderColor: isChecked ? "var(--primary)" : "var(--form-input-border)", backgroundColor: isChecked ? "var(--primary)" : "transparent" }}>
                  {isChecked && (
      ...
      return (
        <div className="flex items-center gap-1.5 flex-wrap">
          {Array.from({ length: scale }).map((_, i) => (
            <button key={i} type="button" onClick={() => onChange(i + 1 === current ? null : i + 1)} className="transition-transform hover:scale-110 active:scale-95">
              <Star className="size-7" style={{ color: "var(--form-star)", fill: i < current ? "var(--form-star)" : "transparent", opacity: i < current ? 1 : 0.3 }} />
            </button>
          ))}
        </div>
      );
      ...
      return (
        <div className="flex gap-3">
          {(["Yes", "No"] as const).map((label) => {
            ...
            return (
              <button key={label} type="button" onClick={() => onChange(isSelected ? null : isYes)}
                className="flex-1 py-3 text-sm font-semibold border-2 transition-colors"
                style={{ borderColor: isSelected ? "var(--primary)" : hasError ? "#ef4444" : "var(--form-input-border)", backgroundColor: isSelected ? "var(--primary)" : "var(--form-input-bg)", color: isSelected ? "var(--primary-foreground)" : "var(--form-label)", borderRadius: "var(--form-input-radius)" }}>
                {label}
              </button>
            );
          })}
...
function WatermarkFooter({ pageFg, pageBg }: { pageFg: string; pageBg: string }) {
  return (
    <div className="shrink-0 flex items-center justify-center gap-1.5 py-3">
      <img src="/logo.svg" alt="Flowform" className="h-3.5 w-auto"
...
function GateScreen({
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ backgroundColor: pageBg, fontFamily }}>
      <div className="w-full max-w-sm rounded-2xl border shadow-xl p-8 flex flex-col items-center gap-5"
        style={{ ...themeVars, backgroundColor: "var(--background)", borderColor: "var(--form-input-border)" }}>
        <div className="size-14 rounded-full flex items-center justify-center" style={{ backgroundColor: iconBg }}>
          <Icon className="size-6" style={{ color: iconColor }} />
        </div>
        <div className="text-center space-y-1">
          <h1 className="text-lg font-bold" style={{ color: "var(--form-label)" }}>{title}</h1>
...
function ConvQuestionPage({
  ...
  pageFg: string; isSubmitting: boolean;
    ...
    return (
      <div className={className}>
        {currentPage?.coverImage ? (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2" style={{ backgroundColor: theme.primaryColor + "15" }}>
    ...
    return (
      <>
        {settings.progressBar.enabled && (
          <div className="mb-4">
            <ProgressIndicator settings={settings} pageIndex={pageIndex} totalPages={pagesLength} pageFg={pageFg} />
          </div>
        )}
  ...
  function NavButtons({ borderTop = false }: { borderTop?: boolean }) {
    return (
      <div className={`flex items-center justify-between ${borderTop ? "pt-4 border-t" : ""}`}
        ...
        {showNextButton && (
          <button type="button" onClick={onNext} disabled={isSubmitting}
            className="flex items-center gap-1.5 px-5 py-2 text-sm font-semibold transition-opacity hover:opacity-90 disabled:opacity-60"
...
export default function PublicFormPage() {
  ...
  // ── Persist access code in state so changing it re-triggers the query ───────
  const [pendingCode, setPendingCode] = useState("");
  const [accessCode, setAccessCode] = useState<string | undefined>(undefined);
  const [codeError, setCodeError] = useState("");
  const [prevAccessCode, setPrevAccessCode] = useState<string | undefined>(undefined);
  ...
  // ── API hooks ────────────────────────────────────────────────────────────────
  const { data, isLoading, isError } = usePublicForm(idOrSlug ?? "", { accessCode });
  ...
  const { mutate: submitResponse, isPending: isSubmitting } = useSubmitResponse(() => {
    // onSuccess handled inline via redirectUrl
  });

  // ── Track view once when form ID is known ────────────────────────────────────
  const trackedRef = useRef(false);
  useEffect(() => {
    if (data?.id && !trackedRef.current) {
      trackedRef.current = true;
      trackView({ formId: data.id });
    }
  }, [data?.id, trackView]);

  // ── Detect wrong access code ─────────────────────────────────────────────────
  useEffect(() => {
    if (accessCode !== undefined && accessCode === prevAccessCode && data?.requiresCode) {
      setCodeError("Incorrect access code. Please try again.");
    }
    if (accessCode !== undefined) setPrevAccessCode(accessCode);
  }, [data, accessCode, prevAccessCode]);
  ...
  // ── Redirect after end page is shown ─────────────────────────────────────────
  useEffect(() => {
    if (!pendingRedirect || view !== "end") return;
    const t = setTimeout(() => router.push(pendingRedirect), 2500);
    return () => clearTimeout(t);
  }, [pendingRedirect, view, router]);
      ...
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            This form doesn&apos;t exist or the link may be incorrect.
          </p>
        </div>
      </div>
    );
  ...
  // Fallback defaults so gate screens always render nicely
  const DEFAULT_THEME: FormTheme = { primaryColor: "#6366f1", backgroundColor: "#ffffff", accentColor: "#818cf8", borderRadius: "rounded" };
  const DEFAULT_FONT: FormFont = { fontFamily: "Inter", fontSize: "md", letterSpacing: "normal" };

  const theme     = (data.theme  as FormTheme | null)  ?? DEFAULT_THEME;
  const font      = (data.font   as FormFont  | null)  ?? DEFAULT_FONT;
  const settings  = data.settings as PublicFormSettings | null;
  ...
  if (data.requiresCode) {
    function handleCodeSubmit() {
      const trimmed = pendingCode.trim().toUpperCase().replace(/[^A-Z0-9]/g, "");
        ...
        >
          <div className="w-full space-y-2">
            <input type="text" value={pendingCode}
              onChange={(e) => { setPendingCode(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "")); setCodeError(""); }}
              onKeyDown={(e) => e.key === "Enter" && handleCodeSubmit()}
              placeholder="ACCESS CODE" maxLength={15}
              className="w-full h-11 px-4 border text-sm font-mono tracking-widest outline-none transition-colors"
              style={{ ...inputStyles(!!codeError), borderRadius: "var(--form-input-radius)" }}
            />
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center space-y-2">
          <p className="text-lg font-semibold">This form isn&apos;t ready yet</p>
  ...
  function validateCurrentPage(): boolean {
    ...
    for (const q of currentPage.questions) {
      ...
      if (q.type === "number" && typeof ans === "number") {
        if (q.config?.min !== undefined && ans < q.config.min) newErrors[q.id] = `Minimum value is ${q.config.min}`;
        else if (q.config?.max !== undefined && ans > q.config.max) newErrors[q.id] = `Maximum value is ${q.config.max}`;
      }
    ...
    if (firstId) setTimeout(() => { document.getElementById(`q-${firstId}`)?.scrollIntoView({ behavior: "smooth", block: "center" }); }, 50);
    ...
    submitResponse(
      {
        ...
        respondentEmail: settings!.collectEmail && emailValue.trim() ? emailValue.trim() : null,
        _hp: honeypot,
      },
      {
        onSuccess: (result) => {
          ...
          if (result.redirectUrl) setPendingRedirect(result.redirectUrl);
        },
      },
    );
  }
  ...
  if (isConversational) {
    return (
      <>
        ...
        {/* honeypot */}
        <input type="text" name="_ff_hp" value={honeypot} readOnly tabIndex={-1} className="hidden" aria-hidden="true" />

        {view === "end" && <EndAnimation key={animationKey} type={content.endPage?.animation ?? "none"} />}

        <div className="h-dvh flex flex-col" style={{ ...rootStyle, backgroundColor: pageBg }}>
            ...
            >
              ...
              {view === "start" && hasStartPage && (
                <div className="flex-1 flex flex-col items-center justify-center gap-6 py-16 px-8 text-center ff-card-enter">
                  ...
                  <div className="space-y-2">
                    <h1 className="text-3xl font-bold" style={{ color: "var(--form-label)" }}>{content.startPage.heading}</h1>
                    {content.startPage.description && (
                      <p className="text-sm max-w-sm" style={{ color: "var(--muted-foreground)" }}>{content.startPage.description}</p>
                    )}
                  </div>
                  {settings.collectEmail && (
                    <div className="w-full max-w-xs space-y-1.5">
                      <input type="email" value={emailValue}
                        ...
                        style={inputStyles(!!emailError)}
                      />
                      {emailError && <p className="text-xs" style={{ color: "#ef4444" }}>{emailError}</p>}
                    </div>
                  )}
                  <button type="button" onClick={handleStart} disabled={isStarting}
                    className="px-8 py-3 text-sm font-semibold transition-opacity hover:opacity-90 disabled:opacity-60 flex items-center gap-2"
                    style={{ backgroundColor: "var(--primary)", color: "var(--primary-foreground)", borderRadius: "var(--form-btn-radius)" }}>
                    {isStarting ? <><Loader2 className="size-4 animate-spin" /> Starting…</> : (content.startPage.buttonLabel || "Start")}
                  </button>
                </div>
              )}
              ...
              {view === "end" && (
                <div className="flex-1 flex flex-col items-center justify-center gap-6 py-16 px-8 text-center ff-card-enter">
                  ...
                  <div className="space-y-2">
                    <h2 className="text-3xl font-bold" style={{ color: "var(--form-label)" }}>{content.endPage?.heading || "Thank you!"}</h2>
                    {content.endPage?.message && (
                      <p className="text-sm max-w-sm" style={{ color: "var(--muted-foreground)" }}>{content.endPage.message}</p>
                    )}
                  </div>
                  {settings.redirectOnComplete && (
                    <a href={settings.redirectOnComplete.url} target="_blank" rel="noopener noreferrer"
  ...
  return (
    <>
      ...
      <div className="min-h-screen flex flex-col" style={{ ...rootStyle, backgroundColor: pageBg }}>
        <NavbarBand settings={settings} pageBg={pageBg} pageFg={pageFg} />
          ...
          <div className="w-full max-w-[560px] flex flex-col" style={themeVars}>
              ...
              <div key={`${view}-${pageIndex}`} className="ff-enter">
                ...
                {view === "start" && hasStartPage && (
                  <div className="flex flex-col items-center gap-6 py-16 text-center px-6">
                    ...
                    <div className="space-y-2">
                      <h1 className="text-2xl font-bold" style={{ color: "var(--form-label)" }}>{content.startPage.heading}</h1>
                      {content.startPage.description && (
                        <p className="text-sm max-w-sm" style={{ color: "var(--muted-foreground)" }}>{content.startPage.description}</p>
                      )}
                    </div>
                    ...
                    <button type="button" onClick={handleStart} disabled={isStarting}
                      className="px-6 py-2.5 text-sm font-semibold transition-opacity hover:opacity-90 disabled:opacity-60 flex items-center gap-2"
                      style={{ backgroundColor: "var(--primary)", color: "var(--primary-foreground)", borderRadius: "var(--form-btn-radius)" }}>
                      {isStarting ? <><Loader2 className="size-4 animate-spin" /> Starting…</> : (content.startPage.buttonLabel || "Start")}
                    </button>
                  </div>
                ...
                {view === "end" && (
                  <div className="flex flex-col items-center gap-6 py-16 text-center px-6">
                    ...
                    <div className="space-y-2">
                      <h2 className="text-2xl font-bold" style={{ color: "var(--form-label)" }}>{content.endPage?.heading || "Thank you!"}</h2>
                      {content.endPage?.message && (
                        <p className="text-sm max-w-sm" style={{ color: "var(--muted-foreground)" }}>{content.endPage.message}</p>
                      )}
                    </div>
              ...
              <div className="mt-3 space-y-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button type="button" onClick={handleNext} disabled={isSubmitting}
                    className="flex items-center gap-1 text-xs font-semibold px-4 py-1.5 rounded-md transition-opacity hover:opacity-90 disabled:opacity-60 shrink-0"
            ...
            {view === "end" && (
              <div className="flex items-center justify-center mt-3">
                <span className="text-xs" style={{ color: pageFg, opacity: 0.45 }}>
                  Response recorded
                </span>
              </div>
            )}
