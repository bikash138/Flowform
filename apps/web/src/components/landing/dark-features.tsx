  ...
  return (
    <div className="w-full rounded-2xl overflow-hidden border border-white/10 shadow-2xl bg-[#1C1610]">
      ...
      <div className="flex items-center gap-3 px-5 py-3 border-b border-white/[0.07] bg-[#0E0A07]">
        ...
        <div className="flex-1 h-5 rounded-md bg-white/8 mx-4" />
        <div className="w-16 h-5 rounded-md bg-[#D9B38C]/20" />
      </div>
      ...
      <div className="flex" style={{ minHeight: 260 }}>
        ...
        <div className="w-36 shrink-0 border-r border-white/[0.07] px-3 py-4 flex flex-col gap-2">
          ...
          {["Short Text", "Long Text", "Dropdown", "Rating", "Date", "File Upload"].map((f) => (
        ...
        <div className="flex-1 px-6 py-5 flex flex-col gap-4">
          ...
          {fields.map((f, i) => (
            <div key={f} className={`flex flex-col gap-1.5 ${i === 1 ? "opacity-60" : ""}`}>
              <div className="h-2.5 rounded" style={{ width: `${f.length * 7}px`, background: "rgba(255,255,255,0.15)" }} />
              <div className={`h-9 w-full rounded-lg border flex items-center px-3 ${i === 0 ? "border-[#D9B38C]/60 bg-[#D9B38C]/5" : "border-white/10 bg-white/5"}`}>
                {i === 0 && <div className="h-2 w-20 rounded bg-[#D9B38C]/30" />}
              </div>
            </div>
          ))}
          <div className="mt-1 w-20 h-8 rounded-lg bg-[#D9B38C]/80 flex items-center justify-center">
            <div className="h-2 w-10 rounded bg-[#17120D]/40" />
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="w-full rounded-2xl overflow-hidden border border-white/10 shadow-2xl bg-[#1C1610]">
      ...
      <div className="px-6 py-4 border-b border-white/[0.07] flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <div className="h-7 w-16 rounded-lg bg-white/8 border border-white/10" />
          <div className="h-7 w-20 rounded-lg bg-[#D9B38C]/20 border border-[#D9B38C]/30" />
        </div>
      </div>
      ...
      <div className="px-6 py-5">
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            ...
            <div key={s.label} className="rounded-xl border border-white/[0.07] bg-white/[0.04] px-3 py-3">
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-10 lg:gap-16 mb-24">
          ...
          <div className="flex-1 flex flex-col items-start">
            ...
            <ul className="flex flex-col gap-2.5 mb-8">
              {["20+ field types", "Conditional logic", "Custom branding", "Mobile-ready by default"].map((item) => (
        ...
        <div className="flex flex-col-reverse lg:flex-row items-center gap-10 lg:gap-16">
          <div className="flex-1 flex flex-col items-start">
            ...
            <ul className="flex flex-col gap-2.5 mb-8">
              {["Real-time submissions", "Completion rate tracking", "CSV export", "Drop-off insights"].map((item) => (
                <li key={item} className="flex items-center gap-2.5 text-sm text-[#9E8E7E]">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#D9B38C] shrink-0" />
                  {item}
                </li>
